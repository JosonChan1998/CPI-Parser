...
class DefaultFormatBundle:
    ...
    def __call__(self, results):
        ...
        if 'aug_img' in results:
            aug_img = results['aug_img']
            if self.img_to_float is True and aug_img.dtype == np.uint8:
                # Normally, image is of uint8 type without normalization.
                # At this time, it needs to be forced to be converted to
                # flot32, otherwise the model training and inference
                # will be wrong. Only used for YOLOX currently .
            ...
            if len(aug_img.shape) < 3:
                aug_img = np.expand_dims(aug_img, -1)
            aug_img = np.ascontiguousarray(aug_img.transpose(2, 0, 1))
            results['aug_img'] = DC(
                to_tensor(aug_img), padding_value=self.pad_val['img'], stack=True)
        ...
        if 'gt_semantic_seg' in results:
            results['gt_semantic_seg'] = DC(
                ...
                stack=True)
        if 'gt_parsings' in results:
            results['gt_parsings'] = DC(
                to_tensor(results['gt_parsings']),
                padding_value=self.pad_val['masks'],
                cpu_only=True)
        if 'gt_parse_labels' in results:
