...
def relative_coordinate_maps(feat_sizes, centers, feat_stride, stride=None, range=8.0):
    if isinstance(stride, int):
        stride = torch.tensor(stride, device=centers.device)
    ...
    relative_coordinates = relative_coordinates / (stride.reshape(-1, 1, 1) * range)
